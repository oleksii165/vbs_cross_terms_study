...
f = ROOT.TFile(f"../workspaces/{opts.prod_dec}_unclip.root" )

# Retrieve workspace from file
w = f.Get("w")

model = w[model[opts.prod_dec]]
if opts.print_model=="yes": model.Print("t")

if opts.print_bins=="yes":
    for binno in range(n_bins[opts.prod_dec]):
        myvar = w[f"{bin_pref[opts.prod_dec]}{binno}"]
        myvar.Print("v")
