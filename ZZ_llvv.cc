...
namespace Rivet {
    ...
    void init() {
        _clips = {"inf", "3000", "2000", "1500", "1000", "700"};
        ...
        // // Cut-flows
        // _cutflows.addCutflow("ZZ_llvv_selections", {"two_lep", "lep_pt", "m_ll", 
        //                                             "dR_ll", "MET", "MET_by_HT", "n_jets"});
    ...
    void analyze(const Event& event) {
        ...
        // std::cout << "++ made it throuth MET things \n" ;

        // // Retrieve clustered jets, sorted by pT, with a minimum pT cut
        // Jets jets;
        // if (_docut==1){
        //     jets = apply<FastJets>(event, "jets_in").jetsByPt(Cuts::pT > dbl(_jcuts["pt_jet"])*GeV);
        // }
        // else {
        //     jets = apply<FastJets>(event, "jets_in").jetsByPt();
        // }
        Jets jets = apply<FastJets>(event, "jets_in").jetsByPt(Cuts::pT > 20*GeV);
        std::cout << "++ extracted jets ok \n" ;

        // idiscardIfAnyDeltaRLess(jets, leptons, 0.2);

        std::cout << "++ rejected overlaps jet-lep ok \n" ;
        std::cout << "num jets jet 1 and 2 pt" << jets.size() << jets[0].pT() << " " << jets[1].pT() << "\n";
        
        int n_jets = jets.size();
        std::cout << "njets" << n_jets;
        if (n_jets < _jcuts["n_jets"]) vetoEvent;  
        _cutflows.fillnext();
        std::cout << "jets after cutflow" << n_jets << "\n";
        ...
        std::cout << "++ made it throuth jet things \n" ;
        ...
        // std::cout << "++ made it throuth getting truth Z's \n" ;
